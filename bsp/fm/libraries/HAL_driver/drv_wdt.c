...
{
    bs_watchdog_t watchdog;
    IWDG_HandleTypeDef hiwdg;
    bs_uint16_t is_start;
};
static struct stm32_wdt_obj stm32_wdt;
static struct bs_watchdog_ops ops;
...
static bs_err_t wdt_control(bs_watchdog_t *wdt, int cmd, void *arg)
{
    ...
    case BS_DEVICE_CTRL_WDT_SET_TIMEOUT:
        ...
        if(LSI_VALUE)
        {
            stm32_wdt.hiwdg.Init.Reload = (*((bs_uint32_t*)arg)) * LSI_VALUE / 256 ;
        }
        else
        {
  ...
  #error "Please define the value of LSI_VALUE!"
    ...
    case BS_DEVICE_CTRL_WDT_GET_TIMEOUT:
        ...
        if(LSI_VALUE)
        {
            (*((bs_uint32_t*)arg)) = stm32_wdt.hiwdg.Init.Reload * 256 / LSI_VALUE;
        }
        else
        {
            LOG_E("Please define the value of LSI_VALUE!");
